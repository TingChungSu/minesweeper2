...
function start(){
	...
	if(n<5 || n>30 || m<5 || m>30){
		$("#warning").text("Warning! Please set height and width between 5 and 30.");
		$("div.alert").slideDown("slow");
		return;
	}else if(num<m*n*0.1){
		$("#warning").text("Warning! Please set more bombs.");
		$("div.alert").slideDown("slow");
	}else if(num>m*n*0.6){
		$("#warning").text("Warning! Please set fewer bombs.");
		$("div.alert").slideDown("slow");
	}
	else{
		$("div.alert").hide(0);
